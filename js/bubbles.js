...
function PointCollection() {
    ...
    this.update = function () {
        for (var i = 0; i < this.points.length; i++) {
            ...
            var dd = (dx * dx) + (dy * dy);
            var d = Math.sqrt(dd)*3;  //you can change "*3" to "*1" so that the bubbles run a bigger cycle. 
...
function Point(x, y, z, size, color) {
    ...
    this.draw = function (bubbleShape, dx, dy) {
        ...
        if (bubbleShape == "square") {
            ...
            ctx.beginPath();
            ctx.arc(this.curPos.x + dx, this.curPos.y + dy, this.radius, 0, Math.PI * 2, true); //PI*1 is semi-cycle, PI*2 is cycle
            ctx.fill();
        }
    };
}
